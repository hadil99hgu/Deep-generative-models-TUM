...
class DDPM(nn.Module):
    ...
    def __init__(self, N: int, type: str, hidden_dim: int, n_layers: int):
        ...
        ##########################################################
        # YOUR CODE HERE
        beta_schedule = torch.linspace(0, 1, N)  # Linearly spaced values between 0 and 1
        beta = 1e-4 + beta_schedule * (2e-2 - 1e-4)  # Linearly increasing beta values
        alpha = 1 - beta
        alpha_bar = torch.cumprod(alpha, dim=0)  # Cumulative product along the diffusion steps
        beta_tilde = beta / alpha_bar  # Element-wise division
        ##########################################################
    ...
    def simplified_loss(
        ...
        # Sample noise
        noise = torch.normal(mean=0.0, std=1.0, size=x0.shape)

        # Compute the noisy data
        z_n = torch.sqrt(self.alpha_bar[n]) * x0 + torch.sqrt(1 - self.alpha_bar[n]) * noise

        # Compute the predicted noise
        predicted_noise = self.model(z_n, n)
